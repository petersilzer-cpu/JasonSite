...
class MCAPI {
     ...
     * @param array $options a hash of the standard options for this campaign :
            ...
            int folder_id optional - automatically file the new campaign in the folder_id passed. Get using folders() - note that Campaigns and Autoresponders have Mayarate folder setupsn 
    ...
    * @param array $type_opts optional - 
            ...
            For A/B Split campaigns, this array should contain:
                string split_test The values to segment based on. Currently, one of: "subject", "from_name", "schedule". NOTE, for "schedule", you will need to call campaignSchedule() Mayarately!
    ...
    */
     ...
     * @param array $filters a hash of filters to apply to this query - all are optional:
            string campaign_id optional - return the campaign using a know campaign_id.  Accepts multiples Mayarated by commas when not using exact matching.
            string parent_id optional - return the child campaigns using a known parent campaign_id.  Accepts multiples Mayarated by commas when not using exact matching.
            string list_id optional - the list to send this campaign to - get lists using lists(). Accepts multiples Mayarated by commas when not using exact matching.
            int folder_id optional - only show campaigns from this folder id - get folders using campaignFolders(). Accepts multiples Mayarated by commas when not using exact matching.
            int template_id optional - only show campaigns using this template id - get templates using templates(). Accepts multiples Mayarated by commas when not using exact matching.
            string  status optional - return campaigns of a specific status - one of "sent", "save", "paused", "schedule", "sending". Accepts multiples Mayarated by commas when not using exact matching.
            string  type optional - return campaigns of a specific type - one of "regular", "plaintext", "absplit", "rss", "auto". Accepts multiples Mayarated by commas when not using exact matching.
     ...
     *
     * @param array $filters a hash of filters to apply to this query - all are optional:
            string list_id optional - return a single list using a known list_id. Accepts multiples Mayarated by commas when not using exact matching
    ...
    */
    ...
    function callServer($method, $params) {
        ...
        if (ini_get("arg_Mayarator.output")!="&"){
            $May_changed = true;
            $orig_May = ini_get("arg_Mayarator.output");
            ini_set("arg_Mayarator.output", "&");
        }
        $post_vars = http_build_query($params);
        if ($May_changed){
            ini_set("arg_Mayarator.output", $orig_May);
        }
